...
import { Search, Book, FileText, Download, Filter, File as FileIcon, ExternalLink, Library, Plus, X, UploadCloud, CheckCircle2 } from 'lucide-react';
import { useNotification } from '../../contexts/NotificationContext';
import { useAuth } from '../../contexts/AuthContext';
import { storage } from '../../lib/firebase';
import { ref, uploadBytesResumable, getDownloadURL } from 'firebase/storage';

type Category = 'All' | 'Course Material' | 'Syllabus' | 'Administrative Form' | 'Computer Science' | 'Engineering' | 'Mathematics' | 'Physics' | 'Humanities' | 'Other';
    ...
    try {
      ...
      uploadTask.on('state_changed', 
        ...
        async () => {
          ...
          if (res.ok) {
            notify({ title: 'Success', message: 'Document uploaded to Digital Library successfully', type: 'success' });
            setShowModal(false);
            fetchResources();
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">Browse, upload, and download academic resources, textbooks, and research papers.</p>
        </div>
        <button 
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3, 4, 5, 6].map(i => (
            <div key={i} className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 animate-pulse h-48"></div>
          ))}
        </div>
        ...
        <div className="bg-white dark:bg-slate-800 p-12 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400 max-w-md mx-auto">
            {searchTerm ? 'We could not find any resources matching your search. Try adjusting your filters or search term.' : 'The digital library is currently empty. Be the first to upload a resource!'}
          </p>
          {!searchTerm && (
            <button 
              onClick={() => setShowModal(true)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredResources.map((resource) => (
            <div key={resource.id} className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-700 overflow-hidden flex flex-col hover:shadow-md transition-all group">
              <div className="p-6 flex-1 flex flex-col">
                ...
                <h3 className="font-bold text-lg text-slate-900 dark:text-white leading-tight mb-2 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors line-clamp-2">
                  {resource.title}
                </h3>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-lg shadow-2xl">
            <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
              <div>
                <h3 className="text-xl font-bold text-slate-800 dark:text-white">Upload to Digital Library</h3>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Share a resource with the school community.</p>
              </div>
              <button 
