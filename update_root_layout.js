                ...
                <div key={link.name} className="relative group">
                  {link.subLinks ? (
                    <>
                      ...
                      <div className="absolute top-full left-0 mt-2 w-56 bg-white border border-gray-100 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform origin-top-left scale-95 group-hover:scale-100 z-50 overflow-hidden">
                        <div className="py-2">
...
const newMobileNav = `{navLinks.map((link) => (
              <div key={link.name}>
                {link.subLinks ? (
                  <>
                    <div className="px-3 py-2 text-base font-bold text-gray-900 border-b border-gray-50 bg-gray-50/50">
                      {link.name}
                    </div>
