...
import { FileSpreadsheet, Save, Search, AlertCircle, ChevronDown, CheckCircle2, Send, Lock } from 'lucide-react';
...
export default function LecturerResultUpload() {
  ...
  const handleScoreChange = (studentId: number, field: 'ca' | 'exam', value: string) => {
    ...
    const maxScore = field === 'ca' ? caRules.caMax : caRules.examMax;
    if (numValue < 0) {
      numValue = 0; 
    ...
    setStudents(prev => prev.map(st => {
      if (st.studentId === studentId) {
        ...
        // Also compute display strings for input so they can type decimal points
        return { 
          ...st, 
          caScore: newCa, 
          examScore: newExam, 
          ...
          <div className="p-4 border-b border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50/50">
            ...
            <div className="flex items-center gap-3 shrink-0">
              <button
                ...
                className="flex items-center gap-2 px-6 py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-medium transition-all"
              >
                        ...
                        <td className="px-6 py-4 text-center align-middle">
                          <input 
                            ...
                            className={\`w-20 text-center px-3 py-2 border rounded-lg outline-none transition-colors \${
                              isReadOnly ? 'bg-slate-50 border-transparent text-slate-500' : 'bg-white border-slate-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 text-slate-900 font-medium'
                            }\`}
                          />
                        </td>
                        <td className="px-6 py-4 text-center align-middle">
                          <input 
                            ...
                            disabled={isReadOnly}
                            className={\`w-20 text-center px-3 py-2 border rounded-lg outline-none transition-colors \${
                              isReadOnly ? 'bg-slate-50 border-transparent text-slate-500' : 'bg-white border-slate-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 text-slate-900 font-medium'
                            }\`}
                          />
                        </td>
                        <td className="px-6 py-4 text-center align-middle">
...
console.log('Successfully rewrote LecturerResultUpload.tsx');
