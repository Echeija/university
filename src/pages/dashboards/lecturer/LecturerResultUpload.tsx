    ...
    const fetchInitData = async () => {
      try {
        const [caRes, coursesRes, gradeRes] = await Promise.all([
          ...
          fetch('/api/grading-rules', { headers: { Authorization: `Bearer ${token}` } })
        ]);
            ...
            <table className="w-full text-left border-collapse whitespace-nowrap">
              <thead>
                <tr className="bg-white border-b border-slate-100">
                  <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-wider sticky left-0 bg-white z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">Student</th>
